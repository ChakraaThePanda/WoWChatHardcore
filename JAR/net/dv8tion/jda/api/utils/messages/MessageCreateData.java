...
SerializableData {
    ...
    protected MessageCreateData(String content, List<MessageEmbed> embeds, List<FileUpload> files2, List<LayoutComponent> components, AllowedMentionsData mentions, MessagePollData poll, boolean tts, int flags) {
        this.content = content;
        this.embeds = Collections.unmodifiableList(embeds);
        this.files = Collections.unmodifiableList(files2);
    ...
    @Nonnull
    public static MessageCreateData fromFiles(@Nonnull Collection<? extends FileUpload> files2) {
        return ((MessageCreateBuilder)new MessageCreateBuilder().setFiles((Collection)files2)).build();
    }

    @Nonnull
    public static MessageCreateData fromFiles(FileUpload ... files2) {
        return ((MessageCreateBuilder)new MessageCreateBuilder().setFiles(files2)).build();
    }
