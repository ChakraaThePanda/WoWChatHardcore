...
implements TypeAdapterFactory {
    ...
    private BoundField createBoundField(final Gson context, Field field2, final Method accessor, String name, final TypeToken<?> fieldType, boolean serialize, boolean deserialize, final boolean blockInaccessible) {
        ...
        return new BoundField(name, field2, serialize, deserialize){
            ...
            @Override
            void write(JsonWriter writer, Object source2) throws IOException, IllegalAccessException {
                Object fieldValue;
                if (!this.serialized) {
                    return;
                }
                if (blockInaccessible) {
                    if (accessor == null) {
                        ReflectiveTypeAdapterFactory.checkAccessible(source2, this.field);
                    } else {
                        ReflectiveTypeAdapterFactory.checkAccessible(source2, accessor);
                    }
                }
                if (accessor != null) {
                    try {
                        fieldValue = accessor.invoke(source2, new Object[0]);
                    }
                    ...
                    fieldValue = this.field.get(source2);
                }
                if (fieldValue == source2) {
                    return;
                }
    ...
    private Map<String, BoundField> getBoundFields(Gson context, TypeToken<?> type, Class<?> raw, boolean blockInaccessible, boolean isRecord) {
        ...
        while (raw != Object.class) {
            ...
            for (Field field2 : fields) {
                boolean serialize = this.includeField(field2, true);
                boolean deserialize = this.includeField(field2, false);
                ...
                if (isRecord) {
                    if (Modifier.isStatic(field2.getModifiers())) {
                        ...
                        accessor = ReflectionHelper.getAccessor(raw, field2);
                        if (!blockInaccessible) {
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    BoundField boundField = this.createBoundField(context, field2, accessor, name, TypeToken.get(fieldType), serialize, deserialize, blockInaccessible);
                    BoundField replaced = result2.put(name, boundField);
                    if (previous != null) continue;
                    previous = replaced;
                }
                if (previous == null) continue;
                throw new IllegalArgumentException("Class " + originalRaw.getName() + " declares multiple JSON fields named '" + previous.name + "'; conflict is caused by fields " + ReflectionHelper.fieldToString(previous.field) + " and " + ReflectionHelper.fieldToString(field2));
            }
    ...
    extends Adapter<T, Object[]> {
        ...
        @Override
        void readField(Object[] accumulator, JsonReader in, BoundField field2) throws IOException {
            Integer componentIndex = this.componentIndices.get(field2.fieldName);
            if (componentIndex == null) {
                throw new IllegalStateException("Could not find the index in the constructor '" + ReflectionHelper.constructorToString(this.constructor) + "' for field with name '" + field2.fieldName + "', unable to determine which argument in the constructor the field corresponds to. This is unexpected behavior, as we expect the RecordComponents to have the same names as the fields in the Java class, and that the order of the RecordComponents is the same as the order of the canonical constructor parameters.");
            }
            field2.readIntoArray(in, componentIndex, accumulator);
        }
    ...
    extends Adapter<T, T> {
        ...
        @Override
        void readField(T accumulator, JsonReader in, BoundField field2) throws IllegalAccessException, IOException {
            field2.readIntoField(in, accumulator);
        }
        ...
        public T read(JsonReader in) throws IOException {
            ...
            try {
                ...
                while (in.hasNext()) {
                    ...
                    this.readField(accumulator, in, field2);
                }
            }
    ...
    static abstract class BoundField {
        ...
        protected BoundField(String name, Field field2, boolean serialized, boolean deserialized) {
            this.name = name;
            this.field = field2;
