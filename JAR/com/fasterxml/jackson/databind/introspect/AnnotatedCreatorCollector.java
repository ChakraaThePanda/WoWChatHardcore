...
extends CollectorBase {
    ...
    private List<AnnotatedConstructor> _findPotentialConstructors(JavaType type, Class<?> primaryMixIn) {
        ...
        if (primaryMixIn != null) {
            ...
            block2: for (ClassUtil.Ctor mixinCtor : ClassUtil.getConstructors(primaryMixIn)) {
                ...
                for (int i = 0; i < ctorCount; ++i) {
                    ...
                    result2.set(i, this.constructNonDefaultConstructor((ClassUtil.Ctor)ctors.get(i), mixinCtor));
                    continue block2;
                }
            }
        }
        ...
        for (int i = 0; i < ctorCount; ++i) {
            AnnotatedConstructor ctor = result2.get(i);
            if (ctor != null) continue;
            result2.set(i, this.constructNonDefaultConstructor((ClassUtil.Ctor)ctors.get(i), null));
        }
        return result2;
    }
        ...
        if (primaryMixIn != null) {
            ...
            block2: for (Method mixinFactory : primaryMixIn.getDeclaredMethods()) {
                ...
                for (int i = 0; i < factoryCount; ++i) {
                    ...
                    result2.set(i, this.constructFactoryCreator((Method)candidates.get(i), initialTypeResCtxt, mixinFactory));
                    continue block2;
                }
            }
        }
        for (int i = 0; i < factoryCount; ++i) {
            AnnotatedMethod factory2 = (AnnotatedMethod)result2.get(i);
            ...
            result2.set(i, this.constructFactoryCreator(candidate, typeResCtxt, null));
        }
        return result2;
    }
    ...
    private AnnotationMap[] collectAnnotations(Annotation[][] mainAnns, Annotation[][] mixinAnns) {
        if (this._collectAnnotations) {
            ...
            AnnotationMap[] result2 = new AnnotationMap[count];
