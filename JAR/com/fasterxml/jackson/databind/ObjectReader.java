...
Serializable {
    ...
    protected Object _bind(JsonParser p, Object valueToUpdate) throws IOException {
        ...
        Object result2 = t == JsonToken.VALUE_NULL ? (valueToUpdate == null ? this._findRootDeserializer(ctxt).getNullValue(ctxt) : valueToUpdate) : (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT ? valueToUpdate : (t == JsonToken.NOT_AVAILABLE ? valueToUpdate : ctxt.readRootValue(p, this._valueType, this._findRootDeserializer(ctxt), this._valueToUpdate)));
        p.clearCurrentToken();
    ...
    protected Object _bindAndClose(JsonParser p0) throws IOException {
        try (JsonParser p = p0;){
            DefaultDeserializationContext ctxt = this.createDeserializationContext(p);
            JsonToken t = this._initForReading(ctxt, p);
            Object result2 = t == JsonToken.VALUE_NULL ? (this._valueToUpdate == null ? this._findRootDeserializer(ctxt).getNullValue(ctxt) : this._valueToUpdate) : (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT ? this._valueToUpdate : (t == JsonToken.NOT_AVAILABLE ? this._valueToUpdate : ctxt.readRootValue(p, this._valueType, this._findRootDeserializer(ctxt), this._valueToUpdate)));
            if (this._config.isEnabled(DeserializationFeature.FAIL_ON_TRAILING_TOKENS)) {
                this._verifyNoTrailingTokens(p, ctxt, this._valueType);
            }
