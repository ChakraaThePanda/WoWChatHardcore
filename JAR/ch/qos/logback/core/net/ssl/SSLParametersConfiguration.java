...
extends ContextAwareBase {
    ...
    private String[] includedStrings(String[] defaults, String included, String excluded) {
        ArrayList<String> values2 = new ArrayList<String>(defaults.length);
        values2.addAll(Arrays.asList(defaults));
        if (included != null) {
            StringCollectionUtil.retainMatching(values2, this.stringToArray(included));
        }
        if (excluded != null) {
            StringCollectionUtil.removeMatching(values2, this.stringToArray(excluded));
        }
