...
extends JaninoEventEvaluatorBase<ILoggingEvent> {
    ...
    protected Object[] getParameterValues(ILoggingEvent loggingEvent) {
        ...
        values2[i++] = Level.DEBUG_INTEGER;
        values2[i++] = Level.INFO_INTEGER;
        values2[i++] = Level.WARN_INTEGER;
        values2[i++] = Level.ERROR_INTEGER;
        values2[i++] = loggingEvent;
        values2[i++] = loggingEvent.getMessage();
        values2[i++] = loggingEvent.getFormattedMessage();
        values2[i++] = loggingEvent.getLoggerName();
        values2[i++] = loggingEvent.getLoggerContextVO();
        values2[i++] = loggingEvent.getLevel().toInteger();
        values2[i++] = loggingEvent.getTimeStamp();
        values2[i++] = loggingEvent.getMDCPropertyMap();
        IThrowableProxy iThrowableProxy = loggingEvent.getThrowableProxy();
        if (iThrowableProxy != null) {
            values2[i++] = iThrowableProxy;
            values2[i++] = iThrowableProxy instanceof ThrowableProxy ? ((ThrowableProxy)iThrowableProxy).getThrowable() : null;
        } else {
            values2[i++] = null;
            values2[i++] = null;
        }
        for (int j = 0; j < matcherListSize; ++j) {
            values2[i++] = (Matcher)this.matcherList.get(j);
        }
        return values2;
    }
