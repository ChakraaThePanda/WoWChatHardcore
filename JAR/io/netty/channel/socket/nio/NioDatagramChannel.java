...
implements DatagramChannel {
    ...
    @Override
    @SuppressJava6Requirement(reason="Usage guarded by java version check")
    public ChannelFuture joinGroup(InetAddress multicastAddress, NetworkInterface networkInterface, InetAddress source2, ChannelPromise promise) {
        NioDatagramChannel.checkJavaVersion();
        ObjectUtil.checkNotNull(multicastAddress, "multicastAddress");
        ObjectUtil.checkNotNull(networkInterface, "networkInterface");
        try {
            MembershipKey key = source2 == null ? this.javaChannel().join(multicastAddress, networkInterface) : this.javaChannel().join(multicastAddress, networkInterface, source2);
    ...
    @Override
    @SuppressJava6Requirement(reason="Usage guarded by java version check")
    public ChannelFuture leaveGroup(InetAddress multicastAddress, NetworkInterface networkInterface, InetAddress source2, ChannelPromise promise) {
        NioDatagramChannel.checkJavaVersion();
        ObjectUtil.checkNotNull(multicastAddress, "multicastAddress");
        ObjectUtil.checkNotNull(networkInterface, "networkInterface");
        NioDatagramChannel nioDatagramChannel = this;
        synchronized (nioDatagramChannel) {
            ...
            if (this.memberships != null && (keys2 = this.memberships.get(multicastAddress)) != null) {
                ...
                while (keyIt.hasNext()) {
                    ...
                    if (!networkInterface.equals(key.networkInterface()) || (source2 != null || key.sourceAddress() != null) && (source2 == null || !source2.equals(key.sourceAddress()))) continue;
                    key.drop();
                    keyIt.remove();
                }
                if (keys2.isEmpty()) {
