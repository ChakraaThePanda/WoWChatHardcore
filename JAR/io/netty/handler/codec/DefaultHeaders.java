...
implements Headers<K, V, T> {
    ...
    @Override
    public T add(K name, Iterable<? extends V> values2) {
        this.validateName(this.nameValidator, true, name);
        int h = this.hashingStrategy.hashCode(name);
        int i = this.index(h);
        for (V v : values2) {
    ...
    @Override
    public T addObject(K name, Iterable<?> values2) {
    ...
    @Override
    public T addObject(K name, Object ... values2) {
        for (Object value : values2) {
            this.addObject(name, value);
        }
        return this.thisT();
    ...
    @Override
    public T set(K name, Iterable<? extends V> values2) {
        this.validateName(this.nameValidator, false, name);
        ObjectUtil.checkNotNull(values2, "values");
        int h = this.hashingStrategy.hashCode(name);
        int i = this.index(h);
        this.remove0(h, i, name);
        for (V v : values2) {
    ...
    @Override
    public T setObject(K name, Iterable<?> values2) {
        this.validateName(this.nameValidator, false, name);
        int h = this.hashingStrategy.hashCode(name);
        int i = this.index(h);
        this.remove0(h, i, name);
    ...
    @Override
    public T setObject(K name, Object ... values2) {
        this.validateName(this.nameValidator, false, name);
        int h = this.hashingStrategy.hashCode(name);
        int i = this.index(h);
        this.remove0(h, i, name);
    ...
    public final boolean equals(Headers<K, V, ?> h2, HashingStrategy<V> valueHashingStrategy) {
        ...
        for (K name : this.names()) {
            ...
            List<V> values2 = this.getAll(name);
            if (otherValues.size() != values2.size()) {
                return false;
            }
            for (int i = 0; i < otherValues.size(); ++i) {
                if (valueHashingStrategy.equals(otherValues.get(i), values2.get(i))) continue;
                return false;
            }
        }
        return true;
    ...
    public final int hashCode(HashingStrategy<V> valueHashingStrategy) {
        ...
        for (K name : this.names()) {
            ...
            for (int i = 0; i < values2.size(); ++i) {
                result2 = 31 * result2 + valueHashingStrategy.hashCode(values2.get(i));
            }
        }
        return result2;
    }
