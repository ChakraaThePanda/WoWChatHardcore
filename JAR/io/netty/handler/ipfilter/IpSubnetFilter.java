...
extends AbstractRemoteAddressFilter<InetSocketAddress> {
    ...
    private static List<IpSubnetFilterRule> sortAndFilter(List<IpSubnetFilterRule> rules) {
        ...
        Iterator<IpSubnetFilterRule> iterator2 = rules.iterator();
        ArrayList<IpSubnetFilterRule> toKeep = new ArrayList<IpSubnetFilterRule>();
        IpSubnetFilterRule ipSubnetFilterRule = parentRule = iterator2.hasNext() ? iterator2.next() : null;
        if (parentRule != null) {
            toKeep.add(parentRule);
        }
        while (iterator2.hasNext()) {
            IpSubnetFilterRule childRule = iterator2.next();
