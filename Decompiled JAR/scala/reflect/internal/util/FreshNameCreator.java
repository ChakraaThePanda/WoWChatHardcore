...
public class FreshNameCreator {
    ...
    public NameFactory newNameFactory(String prefix) {
        String safePrefix = NameTransformer$.MODULE$.encode(prefix);
        AtomicLong counter = this.allocateCounter(safePrefix);
        return new NameFactory(this, safePrefix, counter);
    }

    public String scala$reflect$internal$util$FreshNameCreator$$assemble(String safePrefix, long idx) {
        StringBuilder result2 = new StringBuilder(this.creatorPrefix.length() + safePrefix.length() + this.decimalLength(idx));
        result2.append(this.creatorPrefix);
        result2.append(safePrefix);
        result2.append(idx);
        return new StringBuilder(0).append(this.creatorPrefix).append(safePrefix).append(idx).toString();
    }

    private AtomicLong allocateCounter(String safePrefix) {
        return this.counters().computeIfAbsent(safePrefix, s -> new AtomicLong(0L));
    }

    /*
     * WARNING - void declaration
     */
    public int decimalLength(long i) {
        ...
        if (require_requirement == false) {
            throw new IllegalArgumentException(new StringBuilder(20).append("requirement failed: ").append(BoxesRunTime.boxToLong(i)).toString());
        }
        int ceiling = 10;
        int numDigits = 1;
        int MaxValueLength = 19;
    ...
    public final class NameFactory {
        ...
        private final /* synthetic */ FreshNameCreator $outer;

        public long index() {
            return this.counter.incrementAndGet();
        }

        public String newNameAtIndex(long index) {
        ...
        public NameFactory(FreshNameCreator $outer, String safePrefix, AtomicLong counter) {
