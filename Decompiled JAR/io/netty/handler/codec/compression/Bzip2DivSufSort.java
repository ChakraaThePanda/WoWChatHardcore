    ...
    private void ssMerge(int pa, int first, int middle, int last, int[] buf, int bufoffset, int bufsize, int depth) {
        ...
        while (true) {
            ...
            if (0 < m) {
                ...
                if (i - first <= last - j) {
                    stack[ssize++] = new StackEntry(j, middle + m, last, check2 & 2 | next & 1);
                    middle -= m;
                    last = i;
                    check2 &= 1;
                    continue;
                ...
                stack[ssize++] = new StackEntry(first, middle - m, i, check2 & 1 | next & 2);
                first = j;
                middle += m;
                check2 = check2 & 2 | next & 1;
                continue;
            }
            if ((check2 & 1) != 0) {
                this.ssMergeCheckEqual(pa, depth, first);
        ...
        while (true) {
            ...
            if (limit < 0) {
                if (limit == -1) {
                    ...
                    PartitionResult result2 = this.trPartition(isa, isaD - 1, isaN, first, last, last - 1);
                    a = result2.first;
                    b = result2.last;
                    if (first < a || b < last) {
                        if (a < last) {
