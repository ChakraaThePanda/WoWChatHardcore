...
implements InterfaceHttpPostRequestDecoder {
    ...
    public HttpPostMultipartRequestDecoder(HttpDataFactory factory2, HttpRequest request) {
        this(factory2, request, HttpConstants.DEFAULT_CHARSET);
    }

    public HttpPostMultipartRequestDecoder(HttpDataFactory factory2, HttpRequest request, Charset charset) {
        this.request = ObjectUtil.checkNotNull(request, "request");
        this.charset = ObjectUtil.checkNotNull(charset, "charset");
        this.factory = ObjectUtil.checkNotNull(factory2, "factory");
        ...
        while (!this.skipOneLine()) {
            ...
            if (HttpHeaderNames.CONTENT_DISPOSITION.contentEqualsIgnoreCase(contents[0])) {
                ...
                for (int i = 2; i < contents.length; ++i) {
                    values2 = contents[i].split("=", 2);
                    try {
                        attribute = this.getContentDispositionAttribute((String)values2);
                    }
                    catch (NullPointerException e) {
                        throw new HttpPostRequestDecoder.ErrorDataDecoderException(e);
                    }
            ...
            if (HttpHeaderValues.MULTIPART_MIXED.contentEqualsIgnoreCase(contents[1])) {
                if (this.currentStatus == HttpPostRequestDecoder.MultiPartStatus.DISPOSITION) {
                    String values3 = StringUtil.substringAfter(contents[2], '=');
                    this.multipartMixedBoundary = "--" + values3;
            ...
            for (int i = 1; i < contents.length; ++i) {
                ...
                if (contents[i].regionMatches(true, 0, charsetHeader, 0, charsetHeader.length())) {
                    values2 = StringUtil.substringAfter(contents[i], '=');
                    try {
                        attribute = this.factory.createAttribute(this.request, charsetHeader, HttpPostMultipartRequestDecoder.cleanString((String)values2));
                    }
                    catch (NullPointerException e) {
                        throw new HttpPostRequestDecoder.ErrorDataDecoderException(e);
                    }
                ...
                if (contents[i].contains("=")) {
                    ...
                    String values4 = StringUtil.substringAfter(contents[i], '=');
                    try {
                        attribute5 = this.factory.createAttribute(this.request, HttpPostMultipartRequestDecoder.cleanString(name), values4);
                    }
                    catch (NullPointerException e) {
                        throw new HttpPostRequestDecoder.ErrorDataDecoderException(e);
                    }
    ...
    private Attribute getContentDispositionAttribute(String ... values2) {
        String name = HttpPostMultipartRequestDecoder.cleanString(values2[0]);
    ...
    private static String[] splitMultipartHeader(String sb) {
        ...
        String[] values2 = svalue.indexOf(59) >= 0 ? HttpPostMultipartRequestDecoder.splitMultipartHeaderValues(svalue) : svalue.split(",");
    ...
    private static String[] splitMultipartHeaderValues(String svalue) {
        ArrayList<String> values2 = InternalThreadLocalMap.get().arrayList(1);
        ...
        for (int i = 0; i < svalue.length(); ++i) {
            ...
            if (c != ';') continue;
            values2.add(svalue.substring(start, i));
            start = i + 1;
        }
        values2.add(svalue.substring(start));
