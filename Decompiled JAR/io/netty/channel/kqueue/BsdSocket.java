...
extends Socket {
    ...
    AcceptFilter getAcceptFilter() throws IOException {
        String[] result2 = BsdSocket.getAcceptFilter(this.intValue());
        return result2 == null ? AcceptFilter.PLATFORM_UNSUPPORTED : new AcceptFilter(result2[0], result2[1]);
    }
    ...
    int connectx(InetSocketAddress source2, InetSocketAddress destination, IovArray data, boolean tcpFastOpen) throws IOException {
        int iovDataLength;
        int iovCount;
        long iovAddress;
        ...
        if (source2 == null) {
            ...
            InetAddress sourceInetAddress = source2.getAddress();
        ...
        int result2 = BsdSocket.connectx(this.intValue(), 0, sourceIPv6, sourceAddress, sourceScopeId, sourcePort, destinationIPv6, destinationAddress, destinationScopeId, destinationPort, flags, iovAddress, iovCount, iovDataLength);
        if (result2 == Errors.ERRNO_EINPROGRESS_NEGATIVE) {
            return -iovDataLength;
        }
        if (result2 < 0) {
            return Errors.ioResult("connectx", result2);
        }
        return result2;
    }
