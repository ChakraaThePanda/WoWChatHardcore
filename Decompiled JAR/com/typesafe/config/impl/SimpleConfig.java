...
Serializable {
    ...
    @Override
    public SimpleConfig resolveWith(Config source2) {
        return this.resolveWith(source2, ConfigResolveOptions.defaults());
    }

    @Override
    public SimpleConfig resolveWith(Config source2, ConfigResolveOptions options) {
        AbstractConfigValue resolved = ResolveContext.resolve(this.object, ((SimpleConfig)source2).object, options);
    ...
    private List<BigInteger> getBytesListBigInteger(String path) {
        ArrayList<BigInteger> result2 = new ArrayList<BigInteger>();
    ...
    public long getDuration(String path, TimeUnit unit) {
        AbstractConfigValue v = this.find(path, ConfigValueType.STRING);
        long result2 = unit.convert(SimpleConfig.parseDuration((String)v.unwrapped(), v.origin(), path), TimeUnit.NANOSECONDS);
        return result2;
    }
    ...
    public static BigInteger parseBytes(String input, ConfigOrigin originForException, String pathForException) {
        ...
        try {
            BigInteger result2;
            if (numberString.matches("[0-9]+")) {
                result2 = units.bytes.multiply(new BigInteger(numberString));
            } else {
                BigDecimal resultDecimal = new BigDecimal(units.bytes).multiply(new BigDecimal(numberString));
                result2 = resultDecimal.toBigInteger();
            }
            return result2;
        }
        catch (NumberFormatException e) {
