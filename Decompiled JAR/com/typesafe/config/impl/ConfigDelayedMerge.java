...
ReplaceableMergeStack {
    ...
    @Override
    ResolveResult<? extends AbstractConfigValue> resolveSubstitutions(ResolveContext context, ResolveSource source2) throws AbstractConfigValue.NotPossibleToResolve {
        return ConfigDelayedMerge.resolveSubstitutions(this, this.stack, context, source2);
    }

    static ResolveResult<? extends AbstractConfigValue> resolveSubstitutions(ReplaceableMergeStack replaceable, List<AbstractConfigValue> stack, ResolveContext context, ResolveSource source2) throws AbstractConfigValue.NotPossibleToResolve {
        ...
        for (AbstractConfigValue end : stack) {
            ...
            if (end instanceof Unmergeable) {
                ...
                sourceForEnd = source2.replaceWithinCurrentParent((AbstractConfigValue)((Object)replaceable), remainder);
                if (ConfigImpl.traceSubstitutionsEnabled()) {
                ...
                sourceForEnd = source2.pushParent(replaceable);
            }
            if (ConfigImpl.traceSubstitutionsEnabled()) {
                ConfigImpl.trace(newContext.depth(), "sourceForEnd      =" + sourceForEnd);
            }
            if (ConfigImpl.traceSubstitutionsEnabled()) {
                ConfigImpl.trace(newContext.depth(), "Resolving highest-priority item in delayed merge " + end + " against " + sourceForEnd + " endWasRemoved=" + (source2 != sourceForEnd));
            }
            ResolveResult<? extends AbstractConfigValue> result2 = newContext.resolve(end, sourceForEnd);
            Object resolvedEnd = result2.value;
            newContext = result2.context;
            if (resolvedEnd != null) {
                if (merged == null) {
