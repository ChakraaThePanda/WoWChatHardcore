...
implements Serializable {
    ...
    public String[] findEnumValues(MapperConfig<?> config, AnnotatedClass annotatedClass, Enum<?>[] enumValues, String[] names) {
        ...
        for (AnnotatedField field2 : annotatedClass.fields()) {
            String propValue;
            JsonProperty property = field2.getAnnotation(JsonProperty.class);
            if (property == null || (propValue = property.value()) == null || propValue.isEmpty()) continue;
            enumToPropertyMap.put(field2.getName(), propValue);
        }
        int end = enumValues.length;
        for (int i = 0; i < end; ++i) {
            String defName = enumValues[i].name();
    ...
    public void findEnumAliases(MapperConfig<?> config, AnnotatedClass annotatedClass, Enum<?>[] enumValues, String[][] aliasList) {
        ...
        for (AnnotatedField field2 : annotatedClass.fields()) {
            JsonAlias alias = field2.getAnnotation(JsonAlias.class);
            if (alias == null) continue;
            enumToAliasMap.putIfAbsent(field2.getName(), alias.value());
        }
    ...
    public List<PropertyName> findPropertyAliases(Annotated m) {
        ...
        ArrayList<PropertyName> result2 = new ArrayList<PropertyName>(len);
        for (int i = 0; i < len; ++i) {
            result2.add(PropertyName.construct(strs[i]));
        }
        return result2;
    }
    ...
    public List<NamedType> findSubtypes(Annotated a) {
        ...
        for (JsonSubTypes.Type type : types) {
            result2.add(new NamedType(type.value(), type.name()));
            for (String name : type.names()) {
                result2.add(new NamedType(type.value(), name));
            }
        }
        return result2;
    }
    ...
    private List<NamedType> findSubtypesCheckRepeatedNames(String annotatedTypeName, JsonSubTypes.Type[] types) {
        ...
        for (JsonSubTypes.Type type : types) {
            ...
            result2.add(new NamedType(type.value(), typeName));
            for (String altName : type.names()) {
                ...
                result2.add(new NamedType(type.value(), altName));
            }
        }
        return result2;
    }
