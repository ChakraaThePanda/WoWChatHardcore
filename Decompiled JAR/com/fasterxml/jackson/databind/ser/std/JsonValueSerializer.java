    ...
    protected JsonSerializer<Object> _findDynamicSerializer(SerializerProvider ctxt, Class<?> valueClass) throws JsonMappingException {
        ...
        if (serializer == null) {
            if (this._valueType.hasGenericTypes()) {
                ...
                PropertySerializerMap.SerializerAndMapResult result2 = this._dynamicSerializers.addSerializer(fullType, serializer);
                this._dynamicSerializers = result2.map;
            } else {
                serializer = ctxt.findPrimaryPropertySerializer(valueClass, this._property);
                serializer = JsonValueSerializer._withIgnoreProperties(serializer, this._ignoredProperties);
                PropertySerializerMap.SerializerAndMapResult result3 = this._dynamicSerializers.addSerializer(valueClass, serializer);
                this._dynamicSerializers = result3.map;
            }
        }
        return serializer;
    }
