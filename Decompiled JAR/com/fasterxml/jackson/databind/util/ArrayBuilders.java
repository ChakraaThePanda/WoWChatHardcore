...
public final class ArrayBuilders {
    ...
    public static <T> HashSet<T> arrayToSet(T[] elements) {
        if (elements != null) {
            ...
            HashSet<T> result2 = new HashSet<T>(len);
            for (int i = 0; i < len; ++i) {
    ...
    public static <T> T[] insertInListNoDup(T[] array, T element) {
        ...
        for (int ix = 0; ix < len; ++ix) {
            ...
            Object[] result2 = (Object[])Array.newInstance(array.getClass().getComponentType(), len);
            System.arraycopy(array, 0, result2, 1, ix);
            result2[0] = element;
            int left = len - ++ix;
            if (left > 0) {
                System.arraycopy(array, ix, result2, ix, left);
            }
            return result2;
        }
        Object[] result3 = (Object[])Array.newInstance(array.getClass().getComponentType(), len + 1);
        if (len > 0) {
            System.arraycopy(array, 0, result3, 1, len);
        }
        result3[0] = element;
        return result3;
    }
