...
extends JsonReader {
    ...
    private String nextName(boolean skipName) throws IOException {
        ...
        this.pathNames[this.stackSize - 1] = skipName ? "<skipped>" : result2;
    ...
    public double nextDouble() throws IOException {
        ...
        if (!this.isLenient() && (Double.isNaN(result2) || Double.isInfinite(result2))) {
            throw new MalformedJsonException("JSON forbids NaN and infinities: " + result2);
        }
        this.popStack();
        if (this.stackSize > 0) {
            int n = this.stackSize - 1;
    ...
    private String getPath(boolean usePreviousPath) {
        ...
        for (int i = 0; i < this.stackSize; ++i) {
            if (this.stack[i] instanceof JsonArray) {
                ...
                result2.append('[').append(pathIndex).append(']');
                continue;
            }
