...
implements MessageLite {
    ...
    implements MessageLite.Builder {
        ...
        private static <T> void addAllCheckingNulls(Iterable<T> values2, List<? super T> list) {
            if (list instanceof ArrayList && values2 instanceof Collection) {
                ((ArrayList)list).ensureCapacity(list.size() + ((Collection)values2).size());
            }
            int begin = list.size();
            for (T value : values2) {
                if (value == null) {
        ...
        extends FilterInputStream {
            ...
            public long skip(long n) throws IOException {
                int result2 = (int)super.skip(Math.min(n, (long)this.limit));
                if (result2 >= 0) {
                    this.limit -= result2;
                }
                return result2;
