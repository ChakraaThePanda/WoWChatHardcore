...
final class FieldSet<T extends FieldDescriptorLite<T>> {
    ...
    public Map<T, Object> getAllFields() {
        if (this.hasLazyField) {
            SmallSortedMap<T, Object> result2 = FieldSet.cloneAllFieldsMap(this.fields, false, true);
    ...
    private static <T extends FieldDescriptorLite<T>> SmallSortedMap<T, Object> cloneAllFieldsMap(SmallSortedMap<T, Object> fields, boolean copyList, boolean resolveLazyFields) {
        SmallSortedMap result2 = SmallSortedMap.newFieldMap();
        int n = fields.getNumArrayEntries();
        for (int i = 0; i < n; ++i) {
            FieldSet.cloneFieldEntry(result2, fields.getArrayEntryAt(i), copyList, resolveLazyFields);
        }
        for (Map.Entry<T, Object> entry : fields.getOverflowEntries()) {
            FieldSet.cloneFieldEntry(result2, entry, copyList, resolveLazyFields);
        }
        return result2;
    }
    ...
    static final class Builder<T extends FieldDescriptorLite<T>> {
        ...
        public Map<T, Object> getAllFields() {
            if (this.hasLazyField) {
                SmallSortedMap result2 = FieldSet.cloneAllFieldsMap(this.fields, false, true);
                if (this.fields.isImmutable()) {
                    result2.makeImmutable();
                } else {
