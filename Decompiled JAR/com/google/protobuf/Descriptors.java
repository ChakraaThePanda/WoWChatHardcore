...
public final class Descriptors {
    ...
    private static <T> T binarySearch(T[] array, int size, NumberGetter<T> getter2, int number) {
        int left = 0;
        int right = size - 1;
        while (left <= right) {
            int mid = (left + right) / 2;
            T midValue = array[mid];
            int midValueNumber = getter2.getNumber(midValue);
    ...
    private static final class DescriptorPool {
        ...
        GenericDescriptor findSymbol(String fullName, SearchFilter filter) {
            ...
            if (result2 != null && (filter == SearchFilter.ALL_SYMBOLS || filter == SearchFilter.TYPES_ONLY && this.isType(result2) || filter == SearchFilter.AGGREGATES_ONLY && this.isAggregate(result2))) {
                return result2;
            }
            for (FileDescriptor dependency : this.dependencies) {
                result2 = ((FileDescriptor)dependency).pool.descriptorsByName.get(fullName);
                if (result2 == null || filter != SearchFilter.ALL_SYMBOLS && (filter != SearchFilter.TYPES_ONLY || !this.isType(result2)) && (filter != SearchFilter.AGGREGATES_ONLY || !this.isAggregate(result2))) continue;
                return result2;
            }
            return null;
        }
        ...
        GenericDescriptor lookupSymbol(String name, GenericDescriptor relativeTo, SearchFilter filter) throws DescriptorValidationException {
            ...
            if (name.startsWith(".")) {
                ...
                result2 = this.findSymbol(fullname, filter);
            } else {
                ...
                while (true) {
                    ...
                    if ((dotpos = scopeToTry.lastIndexOf(".")) == -1) {
                        fullname = name;
                        result2 = this.findSymbol(name, filter);
                        break;
                    }
                    ...
                    if (result2 != null) {
                        if (firstPartLength != -1) {
                            ...
                            result2 = this.findSymbol(scopeToTry.toString(), filter);
                        }
    ...
    extends GenericDescriptor {
        ...
        public MethodDescriptor findMethodByName(String name) {
            ...
            if (result2 instanceof MethodDescriptor) {
    ...
    implements Internal.EnumLiteMap<EnumValueDescriptor> {
        ...
        public EnumValueDescriptor findValueByName(String name) {
            GenericDescriptor result2 = this.file.pool.findSymbol(this.fullName + '.' + name);
            if (result2 instanceof EnumValueDescriptor) {
        ...
        public EnumValueDescriptor findValueByNumberCreatingIfUnknown(int number) {
            ...
            synchronized (enumDescriptor) {
                ...
                EnumValueDescriptor enumValueDescriptor = result2 = reference == null ? null : (EnumValueDescriptor)reference.get();
                if (result2 == null) {
                    result2 = new EnumValueDescriptor(this, number);
                    this.unknownValues.put(number, new UnknownEnumValueReference(number, result2));
                }
            }
            return result2;
        }
    ...
    extends GenericDescriptor {
        ...
        public FieldDescriptor findFieldByName(String name) {
            GenericDescriptor result2 = this.file.pool.findSymbol(this.fullName + '.' + name);
            if (result2 instanceof FieldDescriptor) {
        ...
        public EnumDescriptor findEnumTypeByName(String name) {
            GenericDescriptor result2 = this.file.pool.findSymbol(this.fullName + '.' + name);
            if (result2 instanceof EnumDescriptor) {
        ...
        private void validateNoDuplicateFieldNumbers() throws DescriptorValidationException {
            ...
            while (i + 1 < this.fieldsSortedByNumber.length) {
                ...
                FieldDescriptor field2 = this.fieldsSortedByNumber[i + 1];
                if (old.getNumber() == field2.getNumber()) {
    ...
    extends GenericDescriptor {
        ...
        public Descriptor findMessageTypeByName(String name) {
            ...
            if ((result2 = this.pool.findSymbol(name)) instanceof Descriptor && result2.getFile() == this) {
                return (Descriptor)result2;
            }
            return null;
        }
        ...
        public EnumDescriptor findEnumTypeByName(String name) {
            ...
            if ((result2 = this.pool.findSymbol(name)) instanceof EnumDescriptor && result2.getFile() == this) {
                return (EnumDescriptor)result2;
            }
            return null;
        }
        ...
        public ServiceDescriptor findServiceByName(String name) {
            ...
            if ((result2 = this.pool.findSymbol(name)) instanceof ServiceDescriptor && result2.getFile() == this) {
        ...
        public FieldDescriptor findExtensionByName(String name) {
            ...
            if ((result2 = this.pool.findSymbol(name)) instanceof FieldDescriptor && result2.getFile() == this) {
                return (FieldDescriptor)result2;
            }
            return null;
        }
        ...
        private static FileDescriptor buildFrom(DescriptorProtos.FileDescriptorProto proto, FileDescriptor[] dependencies, boolean allowUnknownDependencies, boolean allowUnresolvedFeatures) throws DescriptorValidationException {
            ...
            if (!allowUnresolvedFeatures) {
                result2.resolveAllFeaturesInternal();
            }
            return result2;
        }
