...
extends AbstractMessageLite<MessageType, BuilderType> {
    ...
    static <T extends GeneratedMessageLite<?, ?>> T getDefaultInstance(Class<T> clazz) {
        MessageLite result2 = defaultInstanceMap.get(clazz);
        if (result2 == null) {
            try {
            ...
            result2 = defaultInstanceMap.get(clazz);
        }
        if (result2 == null) {
            result2 = ((GeneratedMessageLite)UnsafeUtil.allocateInstance(clazz)).getDefaultInstanceForType();
            if (result2 == null) {
                throw new IllegalStateException();
            }
            defaultInstanceMap.put(clazz, (GeneratedMessageLite<?, ?>)result2);
        }
        return (T)result2;
    }
    ...
    static <T extends GeneratedMessageLite<T, ?>> T parsePartialFrom(T instance, CodedInputStream input, ExtensionRegistryLite extensionRegistry) throws InvalidProtocolBufferException {
        T result2 = instance.newMutableInstance();
        try {
            Schema<T> schema = Protobuf.getInstance().schemaFor(result2);
            schema.mergeFrom(result2, CodedInputStreamReader.forCodedInput(input), extensionRegistry);
            schema.makeImmutable(result2);
        }
        catch (InvalidProtocolBufferException e) {
            if (e.getThrownFromInputStream()) {
                e = new InvalidProtocolBufferException(e);
            }
            throw e.setUnfinishedMessage((MessageLite)result2);
        }
        catch (UninitializedMessageException e) {
            throw e.asInvalidProtocolBufferException().setUnfinishedMessage((MessageLite)result2);
        }
        catch (IOException e) {
            if (e.getCause() instanceof InvalidProtocolBufferException) {
                throw (InvalidProtocolBufferException)e.getCause();
            }
            throw new InvalidProtocolBufferException(e).setUnfinishedMessage((MessageLite)result2);
        }
    ...
    private static <T extends GeneratedMessageLite<T, ?>> T parsePartialFrom(T defaultInstance, byte[] input, int offset, int length, ExtensionRegistryLite extensionRegistry) throws InvalidProtocolBufferException {
        ...
        T result2 = defaultInstance.newMutableInstance();
        try {
            Schema<T> schema = Protobuf.getInstance().schemaFor(result2);
            schema.mergeFrom(result2, input, offset, offset + length, new ArrayDecoders.Registers(extensionRegistry));
            schema.makeImmutable(result2);
        }
        catch (InvalidProtocolBufferException e) {
            if (e.getThrownFromInputStream()) {
            ...
            throw InvalidProtocolBufferException.truncatedMessage().setUnfinishedMessage((MessageLite)result2);
        }
        return result2;
    }
