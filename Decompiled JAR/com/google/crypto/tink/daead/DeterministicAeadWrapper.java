...
implements PrimitiveWrapper<DeterministicAead, DeterministicAead> {
    ...
    @Override
    public DeterministicAead wrap(PrimitiveSet<DeterministicAead> primitives) throws GeneralSecurityException {
        ...
        return new WrappedDeterministicAead(new DeterministicAeadWithId(primitives.getPrimitiveForEntry(handle.getPrimary()), handle.getPrimary().getId()), builder.build(), encLogger, decLogger);
    }
    ...
    implements DeterministicAead {
        ...
        public byte[] encryptDeterministically(byte[] plaintext, byte[] associatedData) throws GeneralSecurityException {
            try {
                byte[] result2 = this.primary.daead.encryptDeterministically(plaintext, associatedData);
        ...
        public byte[] decryptDeterministically(byte[] ciphertext, byte[] associatedData) throws GeneralSecurityException {
            for (DeterministicAeadWithId aeadWithId : this.allDaeads.getAllWithMatchingPrefix(ciphertext)) {
                try {
                    byte[] result2 = aeadWithId.daead.decryptDeterministically(ciphertext, associatedData);
