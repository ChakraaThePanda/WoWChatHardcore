...
implements Mac {
    ...
    private PrfMac(AesCmacKey key) throws GeneralSecurityException {
        ...
        this.plaintextLegacySuffix = key.getParameters().getVariant().equals(AesCmacParameters.Variant.LEGACY) ? Arrays.copyOf(FORMAT_VERSION, FORMAT_VERSION.length) : new byte[0];
    }
    ...
    private PrfMac(HmacKey key) throws GeneralSecurityException {
        ...
        this.tagSize = key.getParameters().getCryptographicTagSizeBytes();
        this.outputPrefix = key.getOutputPrefix().toByteArray();
        this.plaintextLegacySuffix = key.getParameters().getVariant().equals(HmacParameters.Variant.LEGACY) ? Arrays.copyOf(FORMAT_VERSION, FORMAT_VERSION.length) : new byte[0];
    }
