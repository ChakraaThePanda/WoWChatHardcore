...
implements PrimitiveWrapper<KeyDeriver, KeysetDeriver> {
    ...
    public KeysetDeriver wrap(PrimitiveSet<KeyDeriver> primitiveSet) throws GeneralSecurityException {
        KeysetDeriverWrapper.validate(primitiveSet.getKeysetHandle());
        ...
        for (int i = 0; i < keysetHandle.size(); ++i) {
            ...
            derivers.add(new DeriverWithId(primitiveSet.getPrimitiveForEntry(entry), entry.getId(), entry.isPrimary()));
        }
    ...
    implements KeysetDeriver {
        ...
        private static KeysetHandle.Builder.Entry deriveAndGetEntry(byte[] salt, DeriverWithId deriverWithId) throws GeneralSecurityException {
            ...
            KeysetHandle.Builder.Entry result2 = KeysetHandle.importKey(key);
            result2.withFixedId(deriverWithId.id);
