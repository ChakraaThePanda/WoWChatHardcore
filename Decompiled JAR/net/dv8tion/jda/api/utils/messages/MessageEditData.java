...
SerializableData {
    ...
    protected MessageEditData(int configuredFields, int messageFlags, boolean isReplace, String content, List<MessageEmbed> embeds, List<AttachedFile> files2, List<LayoutComponent> components, AllowedMentionsData mentions) {
    ...
    @Nonnull
    public static MessageEditData fromFiles(@Nonnull Collection<? extends FileUpload> files2) {
        return ((MessageEditBuilder)new MessageEditBuilder().setFiles(files2)).build();
    }

    @Nonnull
    public static MessageEditData fromFiles(FileUpload ... files2) {
        return ((MessageEditBuilder)new MessageEditBuilder().setFiles(files2)).build();
    }
