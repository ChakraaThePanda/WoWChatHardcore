...
extends SocketHandler {
    ...
    private void handleHideChildThreads(IThreadContainer channel) {
        ...
        for (ThreadChannel thread2 : threads) {
            GuildImpl guild = (GuildImpl)channel.getGuild();
            ...
            try {
                ...
                try {
                    threadView.remove(thread2.getType(), thread2.getIdLong());
                    guildThreadView.remove(thread2);
                }
                finally {
        ...
        for (ThreadChannel thread2 : threads) {
            this.api.handleEvent(new ThreadHiddenEvent(this.api, this.responseNumber, thread2));
        }
    }
