...
implements RestAction<T> {
    ...
    private static Consumer<? super Throwable> DEFAULT_FAILURE = t -> {
        if (t instanceof CancellationException || t instanceof TimeoutException) {
            ...
            LOG.error("RestAction queue returned failure", (Throwable)t);
    ...
    @Nonnull
    protected RequestBody getMultipartBody(@Nonnull List<? extends AttachedFile> files2, @Nonnull DataObject json) {
        RequestBody payloadJson = this.getRequestBody(json);
        if (files2.isEmpty()) {
            return payloadJson;
        }
        return AttachedFile.createMultipartBody(files2, payloadJson).build();
    }
