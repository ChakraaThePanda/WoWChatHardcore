...
public class Decoder {
    ...
    public short[] decodeFromOpus(AudioPacket decryptedPacket) {
        ...
        if (decryptedPacket == null) {
            result2 = Opus.INSTANCE.opus_decode(this.opusDecoder, null, 0, decoded, 960, 0);
            this.lastSeq = (char)65535;
            this.lastTimestamp = -1;
        } else {
            ...
            result2 = Opus.INSTANCE.opus_decode(this.opusDecoder, buf, buf.length, decoded, 960, 0);
        }
        if (result2 < 0) {
            this.handleDecodeError(result2);
            return null;
        }
        short[] audio = new short[result2 * 2];
        decoded.get(audio);
        return audio;
    }

    private void handleDecodeError(int result2) {
