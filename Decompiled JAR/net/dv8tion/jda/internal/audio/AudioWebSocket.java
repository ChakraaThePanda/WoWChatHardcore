...
extends WebSocketAdapter {
    ...
    protected void close(ConnectionStatus closeStatus) {
        ...
        this.locked(manager -> {
            ...
            } else if ((status == ConnectionStatus.ERROR_LOST_CONNECTION || status == ConnectionStatus.DISCONNECTED_KICKED_FROM_CHANNEL) && (connGuild = api2.getGuildById(this.guild.getIdLong())) != null && (channel = (AudioChannel)connGuild.getGuildChannelById(this.audioConnection.getChannel().getIdLong())) == null) {
    ...
    public void onThreadCreated(WebSocket websocket, ThreadType threadType, Thread thread2) {
        ...
        switch (threadType) {
            case CONNECT_THREAD: {
                thread2.setName(identifier + " AudioWS-ConnectThread (guildId: " + guildId + ')');
                break;
            }
            case FINISH_THREAD: {
                thread2.setName(identifier + " AudioWS-FinishThread (guildId: " + guildId + ')');
                break;
            }
            case WRITING_THREAD: {
                thread2.setName(identifier + " AudioWS-WriteThread (guildId: " + guildId + ')');
                break;
            }
            case READING_THREAD: {
                thread2.setName(identifier + " AudioWS-ReadThread (guildId: " + guildId + ')');
                break;
            }
            default: {
                thread2.setName(identifier + " AudioWS-" + (Object)((Object)threadType) + " (guildId: " + guildId + ')');
            }
        }
    }
