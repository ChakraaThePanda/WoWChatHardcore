...
implements Bag<E> {
    ...
    public <T> T[] toArray(T[] array) {
        ...
        if (array.length < size) {
            Object[] unchecked2 = (Object[])Array.newInstance(array.getClass().getComponentType(), size);
            array = unchecked2;
        }
        int i = 0;
        for (E current : this.map.keySet()) {
            for (int index = this.getCount(current); index > 0; --index) {
                E unchecked3 = current;
                array[i++] = unchecked3;
            }
        }
