...
implements Serializable {
    ...
    protected List<BeanPropertyWriter> findBeanProperties(SerializerProvider prov, BeanDescription beanDesc, BeanSerializerBuilder builder) throws JsonMappingException {
        ...
        ArrayList<BeanPropertyWriter> result2 = new ArrayList<BeanPropertyWriter>(properties.size());
        for (BeanPropertyDefinition property : properties) {
            ...
            if (accessor instanceof AnnotatedMethod) {
                result2.add(this._constructWriter(prov, property, pb, staticTyping, (AnnotatedMethod)accessor));
                continue;
            }
            result2.add(this._constructWriter(prov, property, pb, staticTyping, (AnnotatedField)accessor));
        }
        return result2;
    }
    ...
    protected void removeIgnorableTypes(SerializationConfig config, BeanDescription beanDesc, List<BeanPropertyDefinition> properties) {
        ...
        while (it.hasNext()) {
            ...
            if (result2 == null) {
                ...
                AnnotatedClass ac;
                result2 = config.getConfigOverride(type).getIsIgnoredType();
                if (result2 == null && (result2 = intr.isIgnorableType(ac = (desc = config.introspectClassAnnotations(type)).getClassInfo())) == null) {
