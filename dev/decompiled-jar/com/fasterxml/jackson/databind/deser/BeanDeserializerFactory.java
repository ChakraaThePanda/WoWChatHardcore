...
implements Serializable {
    ...
    public JsonDeserializer<Object> buildThrowableDeserializer(DeserializationContext ctxt, JavaType type, BeanDescription beanDesc) throws JsonMappingException {
        ...
        if ((am = beanDesc.findMethod("initCause", INIT_CAUSE_PARAMS)) != null) {
            ...
            if (causeCreatorProp instanceof CreatorProperty) {
                ...
                if ((prop = this.constructSettableProperty(ctxt, beanDesc, propDef = SimpleBeanPropertyDefinition.construct(ctxt.getConfig(), am, new PropertyName(name)), am.getParameterType(0))) != null) {
    ...
    protected void addBeanProps(DeserializationContext ctxt, BeanDescription beanDesc, BeanDeserializerBuilder builder) throws JsonMappingException {
        ...
        if ((anySetter = this._resolveAnySetter(ctxt, beanDesc, creatorProps)) != null) {
        ...
        for (BeanPropertyDefinition propDef : propDefs) {
            ...
            if (propDef.hasSetter()) {
                AnnotatedMethod setter2 = propDef.getSetter();
                propertyType = setter2.getParameterType(0);
                ...
                AnnotatedMethod getter2 = propDef.getGetter();
                if (getter2 != null) {
                    PropertyMetadata md;
                    if (useGettersAsSetters && this._isSetterlessType(getter2.getRawType())) {
    ...
    private SettableAnyProperty _resolveAnySetter(DeserializationContext ctxt, BeanDescription beanDesc, SettableBeanProperty[] creatorProps) throws JsonMappingException {
        ...
        if (creatorProps != null) {
            for (SettableBeanProperty prop : creatorProps) {
                ...
                if (member == null || !Boolean.TRUE.equals(ctxt.getAnnotationIntrospector().hasAnySetter(member))) continue;
                return this.constructAnySetter(ctxt, beanDesc, member);
            }
        }
        if ((anySetter = beanDesc.findAnySetterAccessor()) != null) {
            return this.constructAnySetter(ctxt, beanDesc, anySetter);
        }
        return null;
    }
    ...
    protected SettableBeanProperty constructSetterlessProperty(DeserializationContext ctxt, BeanDescription beanDesc, BeanPropertyDefinition propDef) throws JsonMappingException {
        AnnotatedMethod getter2 = propDef.getGetter();
        JavaType type = this.resolveMemberAndTypeAnnotations(ctxt, getter2, getter2.getType());
        TypeDeserializer typeDeser = (TypeDeserializer)type.getTypeHandler();
        SettableBeanProperty prop = new SetterlessProperty(propDef, type, typeDeser, beanDesc.getClassAnnotations(), getter2);
        JsonDeserializer<?> deser = this.findDeserializerFromAnnotation(ctxt, getter2);
        if (deser == null) {
            deser = (JsonDeserializer<?>)type.getValueHandler();
        }
        if (deser != null) {
