...
implements MessageInfoFactory {
    ...
    private static MessageInfo convert(Class<?> messageType, Descriptors.Descriptor messageDescriptor) {
        ...
        for (int i = 0; i < fieldDescriptors.size(); ++i) {
            ...
            Field field2 = DescriptorMessageInfoFactory.field(messageType, fd);
            ...
            if (!fd.hasPresence()) {
                ...
                if (fd.isMapField()) {
                    ...
                    fieldImplicitPresence = FieldInfo.forMapField(field2, number, SchemaUtil.getMapDefaultEntry(messageType, fd.getName()), enumVerifier);
                } else {
                    fieldImplicitPresence = fd.isRepeated() && fd.getJavaType() == Descriptors.FieldDescriptor.JavaType.MESSAGE ? FieldInfo.forRepeatedMessageField(field2, number, type, DescriptorMessageInfoFactory.getTypeForRepeatedMessageField(messageType, fd)) : (fd.isPacked() ? (enumVerifier != null ? FieldInfo.forPackedFieldWithEnumVerifier(field2, number, type, enumVerifier, DescriptorMessageInfoFactory.cachedSizeField(messageType, fd)) : FieldInfo.forPackedField(field2, number, type, DescriptorMessageInfoFactory.cachedSizeField(messageType, fd))) : (enumVerifier != null ? FieldInfo.forFieldWithEnumVerifier(field2, number, type, enumVerifier) : FieldInfo.forField(field2, number, type, enforceUtf8)));
                }
            ...
            FieldInfo fieldExplicitPresence = fd.isRequired() ? FieldInfo.forLegacyRequiredField(field2, number, type, bitField, presenceMask, enforceUtf8, enumVerifier) : FieldInfo.forExplicitPresenceField(field2, number, type, bitField, presenceMask, enforceUtf8, enumVerifier);
    ...
    private static Class<?> getOneofStoredTypeForMessage(Class<?> messageType, Descriptors.FieldDescriptor fd) {
        try {
            String name = fd.getType() == Descriptors.FieldDescriptor.Type.GROUP ? fd.getMessageType().getName() : fd.getName();
            Method getter2 = messageType.getDeclaredMethod(DescriptorMessageInfoFactory.getterForField(name), new Class[0]);
            return getter2.getReturnType();
        }
        catch (Exception e) {
    ...
    private static Class<?> getTypeForRepeatedMessageField(Class<?> messageType, Descriptors.FieldDescriptor fd) {
        try {
            String name = fd.getType() == Descriptors.FieldDescriptor.Type.GROUP ? fd.getMessageType().getName() : fd.getName();
            Method getter2 = messageType.getDeclaredMethod(DescriptorMessageInfoFactory.getterForField(name), Integer.TYPE);
            return getter2.getReturnType();
        }
        catch (Exception e) {
            throw new RuntimeException(e);
    ...
    static class IsInitializedCheckAnalyzer {
        ...
        private Node dfs(Descriptors.Descriptor descriptor) {
            Node result2 = new Node(descriptor, this.index++);
            this.stack.push(result2);
            this.nodeCache.put(descriptor, result2);
            for (Descriptors.FieldDescriptor field2 : descriptor.getFields()) {
                ...
                Node child = this.nodeCache.get(field2.getMessageType());
                if (child == null) {
                    child = this.dfs(field2.getMessageType());
                    result2.lowLink = Math.min(result2.lowLink, child.lowLink);
                    continue;
                }
                if (child.component != null) continue;
                result2.lowLink = Math.min(result2.lowLink, child.lowLink);
            }
            if (result2.index == result2.lowLink) {
                Node node;
        ...
        private void analyze(StronglyConnectedComponent component) {
            ...
            block0: for (Descriptors.Descriptor descriptor : component.messages) {
                ...
                for (Descriptors.FieldDescriptor field2 : descriptor.getFields()) {
                    ...
                    if (field2.getJavaType() != Descriptors.FieldDescriptor.JavaType.MESSAGE) continue;
                    Node node = this.nodeCache.get(field2.getMessageType());
