...
implements KeyDeriver {
    ...
    public Key deriveKey(byte[] salt) throws GeneralSecurityException {
        InputStream inputStream2 = this.prf.computePrf(salt);
        return MutableKeyDerivationRegistry.globalInstance().createKeyFromRandomness(this.key.getParameters().getDerivedKeyParameters(), inputStream2, this.key.getIdRequirementOrNull(), InsecureSecretKeyAccess.get());
    }
}
