...
public final class Ed25519 {
    ...
    private static XYZ scalarMultWithBase(byte[] a) {
        ...
        XYZ result2 = new XYZ(ret);
        if (!result2.isOnCurve()) {
    ...
    private static long load3(byte[] in, int idx) {
        long result2 = (long)in[idx] & 0xFFL;
        result2 |= (long)(in[idx + 1] & 0xFF) << 8;
        return result2 |= (long)(in[idx + 2] & 0xFF) << 16;
    }

    private static long load4(byte[] in, int idx) {
        long result2 = Ed25519.load3(in, idx);
        return result2 |= (long)(in[idx + 3] & 0xFF) << 24;
    }
    ...
    private static class XYZT {
        ...
        private static XYZT fromBytesNegateVarTime(byte[] s) throws GeneralSecurityException {
            ...
            Field25519.sub(check2, vxx, u);
            if (Ed25519.isNonZeroVarTime(check2)) {
                Field25519.sum(check2, vxx, u);
                if (Ed25519.isNonZeroVarTime(check2)) {
