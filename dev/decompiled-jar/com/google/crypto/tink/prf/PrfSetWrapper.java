...
implements PrimitiveWrapper<Prf, PrfSet> {
    ...
    public PrfSet wrap(PrimitiveSet<Prf> set) throws GeneralSecurityException {
        ...
        HashMap<Integer, WrappedPrfSet.PrfWithMonitoring> mutablePrfMap = new HashMap<Integer, WrappedPrfSet.PrfWithMonitoring>();
        for (int i = 0; i < keysetHandle.size(); ++i) {
            LegacyProtoKey legacyProtoKey;
            KeysetHandleInterface.Entry entry = keysetHandle.getAt(i);
            if (entry.getKey() instanceof LegacyProtoKey && (legacyProtoKey = (LegacyProtoKey)entry.getKey()).getOutputPrefix().size() != 0) {
