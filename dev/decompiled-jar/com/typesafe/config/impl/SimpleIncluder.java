...
implements FullIncluder {
    ...
    static ConfigObject includeWithoutFallback(ConfigIncludeContext context, String name) {
        ...
        RelativeNameSource source2 = new RelativeNameSource(context);
        return SimpleIncluder.fromBasename(source2, name, context.parseOptions());
    }
    ...
    static ConfigObject fromBasename(NameSource source2, String name, ConfigParseOptions options) {
        ConfigObject obj;
        if (name.endsWith(".conf") || name.endsWith(".json") || name.endsWith(".properties")) {
            ConfigParseable p = source2.nameToParseable(name, options);
            obj = p.parse(p.options().setAllowMissing(options.getAllowMissing()));
        } else {
            ConfigObject parsed;
            ConfigParseable confHandle = source2.nameToParseable(name + ".conf", options);
            ConfigParseable jsonHandle = source2.nameToParseable(name + ".json", options);
            ConfigParseable propsHandle = source2.nameToParseable(name + ".properties", options);
