...
implements Unmergeable {
    ...
    @Override
    ResolveResult<? extends AbstractConfigValue> resolveSubstitutions(ResolveContext context, ResolveSource source2) {
        AbstractConfigValue v;
        ResolveContext newContext = context.addCycleMarker(this);
        try {
            ResolveSource.ResultWithPath resultWithPath = source2.lookupSubst(newContext, this.expr, this.prefixLength);
            newContext = resultWithPath.result.context;
            if (resultWithPath.result.value != null) {
                if (ConfigImpl.traceSubstitutionsEnabled()) {
                    ConfigImpl.trace(newContext.depth(), "recursively resolving " + resultWithPath + " which was the resolution of " + this.expr + " against " + source2);
                }
                ...
                ResolveResult<? extends AbstractConfigValue> result2 = newContext.resolve((AbstractConfigValue)resultWithPath.result.value, recursiveResolveSource);
                v = result2.value;
