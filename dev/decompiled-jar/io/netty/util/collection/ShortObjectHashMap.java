...
implements ShortObjectMap<V> {
    ...
    public void putAll(Map<? extends Short, ? extends V> sourceMap) {
        if (sourceMap instanceof ShortObjectHashMap) {
            ShortObjectHashMap source2 = (ShortObjectHashMap)sourceMap;
            for (int i = 0; i < source2.values.length; ++i) {
                V sourceValue = source2.values[i];
                if (sourceValue == null) continue;
                this.put(source2.keys[i], sourceValue);
            }
            return;
        }
