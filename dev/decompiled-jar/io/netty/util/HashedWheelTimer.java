...
implements Timer {
    ...
    public Timeout newTimeout(TimerTask task, long delay, TimeUnit unit) {
        ...
        HashedWheelTimeout timeout2 = new HashedWheelTimeout(this, task, deadline);
    ...
    private static final class HashedWheelBucket {
        ...
        public void addTimeout(HashedWheelTimeout timeout2) {
            assert (timeout2.bucket == null);
            timeout2.bucket = this;
            if (this.head == null) {
                this.head = this.tail = timeout2;
            } else {
                this.tail.next = timeout2;
                timeout2.prev = this.tail;
                this.tail = timeout2;
            }
        }
        ...
        public void expireTimeouts(long deadline) {
            HashedWheelTimeout timeout2 = this.head;
            while (timeout2 != null) {
                HashedWheelTimeout next = timeout2.next;
                if (timeout2.remainingRounds <= 0L) {
                    next = this.remove(timeout2);
                    if (timeout2.deadline > deadline) throw new IllegalStateException(String.format("timeout.deadline (%d) > deadline (%d)", timeout2.deadline, deadline));
                    timeout2.expire();
                } else if (timeout2.isCancelled()) {
                    next = this.remove(timeout2);
                } else {
                    --timeout2.remainingRounds;
                }
                timeout2 = next;
            }
        }

        public HashedWheelTimeout remove(HashedWheelTimeout timeout2) {
            HashedWheelTimeout next = timeout2.next;
            if (timeout2.prev != null) {
                timeout2.prev.next = next;
            }
            if (timeout2.next != null) {
                timeout2.next.prev = timeout2.prev;
            }
            if (timeout2 == this.head) {
                if (timeout2 == this.tail) {
                    this.tail = null;
                    this.head = null;
                } else {
                    this.head = next;
                }
            } else if (timeout2 == this.tail) {
                this.tail = timeout2.prev;
            }
            timeout2.prev = null;
            timeout2.next = null;
            timeout2.bucket = null;
            timeout2.timer.pendingTimeouts.decrementAndGet();
            return next;
        }

        public void clearTimeouts(Set<Timeout> set) {
            HashedWheelTimeout timeout2;
            while ((timeout2 = this.pollTimeout()) != null) {
                if (timeout2.isExpired() || timeout2.isCancelled()) continue;
                set.add(timeout2);
            }
            return;
        }
    ...
    implements Runnable {
        ...
        public void run() {
            ...
            while ((timeout2 = (HashedWheelTimeout)HashedWheelTimer.this.timeouts.poll()) != null) {
                if (timeout2.isCancelled()) continue;
                this.unprocessedTimeouts.add(timeout2);
            }
            this.processCancelledTasks();
        }
        ...
        private void transferTimeoutsToBuckets() {
            HashedWheelTimeout timeout2;
            for (int i = 0; i < 100000 && (timeout2 = (HashedWheelTimeout)HashedWheelTimer.this.timeouts.poll()) != null; ++i) {
                if (timeout2.state() == 1) continue;
                long calculated = timeout2.deadline / HashedWheelTimer.this.tickDuration;
                timeout2.remainingRounds = (calculated - this.tick) / (long)HashedWheelTimer.this.wheel.length;
                ...
                bucket.addTimeout(timeout2);
            }
        }

        private void processCancelledTasks() {
            HashedWheelTimeout timeout2;
            while ((timeout2 = (HashedWheelTimeout)HashedWheelTimer.this.cancelledTimeouts.poll()) != null) {
                try {
                    timeout2.remove();
                }
                catch (Throwable t) {
