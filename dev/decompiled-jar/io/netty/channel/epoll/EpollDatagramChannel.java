...
implements DatagramChannel {
    ...
    @Override
    public ChannelFuture joinGroup(final InetAddress multicastAddress, final NetworkInterface networkInterface, final InetAddress source2, final ChannelPromise promise) {
        ObjectUtil.checkNotNull(multicastAddress, "multicastAddress");
        ObjectUtil.checkNotNull(networkInterface, "networkInterface");
        if (this.eventLoop().inEventLoop()) {
            this.joinGroup0(multicastAddress, networkInterface, source2, promise);
        } else {
            this.eventLoop().execute(new Runnable(){

                @Override
                public void run() {
                    EpollDatagramChannel.this.joinGroup0(multicastAddress, networkInterface, source2, promise);
                }
            });
        }
        return promise;
    ...
    private void joinGroup0(InetAddress multicastAddress, NetworkInterface networkInterface, InetAddress source2, ChannelPromise promise) {
        assert (this.eventLoop().inEventLoop());
        try {
            this.socket.joinGroup(multicastAddress, networkInterface, source2);
            promise.setSuccess();
        }
        catch (IOException e) {
            promise.setFailure(e);
    ...
    @Override
    public ChannelFuture leaveGroup(final InetAddress multicastAddress, final NetworkInterface networkInterface, final InetAddress source2, final ChannelPromise promise) {
        ObjectUtil.checkNotNull(multicastAddress, "multicastAddress");
        ObjectUtil.checkNotNull(networkInterface, "networkInterface");
        if (this.eventLoop().inEventLoop()) {
            this.leaveGroup0(multicastAddress, networkInterface, source2, promise);
        } else {
            this.eventLoop().execute(new Runnable(){

                @Override
                public void run() {
                    EpollDatagramChannel.this.leaveGroup0(multicastAddress, networkInterface, source2, promise);
                }
            });
        }
        return promise;
    ...
    private void leaveGroup0(InetAddress multicastAddress, NetworkInterface networkInterface, InetAddress source2, ChannelPromise promise) {
        assert (this.eventLoop().inEventLoop());
        try {
            this.socket.leaveGroup(multicastAddress, networkInterface, source2);
            promise.setSuccess();
        }
        catch (IOException e) {
            promise.setFailure(e);
