...
public final class Native {
    ...
    public static int epollWait(FileDescriptor epollFd, EpollEventArray events, FileDescriptor timerFd, int timeoutSec, int timeoutNs) throws IOException {
        long result2 = Native.epollWait(epollFd, events, timerFd, timeoutSec, timeoutNs, -1L);
        return Native.epollReady(result2);
    }
    ...
    static long epollWait(FileDescriptor epollFd, EpollEventArray events, FileDescriptor timerFd, int timeoutSec, int timeoutNs, long millisThreshold) throws IOException {
        ...
        if ((ready = Native.epollReady(result2 = Native.epollWait0(epollFd.intValue(), events.memoryAddress(), events.length(), timerFd.intValue(), timeoutSec, timeoutNs, millisThreshold))) < 0) {
            throw Errors.newIOException("epoll_wait", ready);
        }
        return result2;
    }

    static int epollReady(long result2) {
        return (int)(result2 >> 32);
    }

    static boolean epollTimerWasUsed(long result2) {
        return (result2 & 0xFFL) != 0L;
    }
