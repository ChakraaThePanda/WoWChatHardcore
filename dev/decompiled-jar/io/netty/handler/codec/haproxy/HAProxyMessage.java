...
extends AbstractReferenceCounted {
    ...
    private static List<HAProxyTLV> readTlvs(ByteBuf header) {
        HAProxyTLV haProxyTLV = HAProxyMessage.readNextTLV(header, 0);
        if (haProxyTLV == null) {
        ...
        } while ((haProxyTLV = HAProxyMessage.readNextTLV(header, 0)) != null);
        return haProxyTLVs;
    }
        ...
        switch (type) {
            case PP2_TYPE_SSL: {
                ...
                if (byteBuf.readableBytes() >= 4) {
                    ...
                    while ((haProxyTLV = HAProxyMessage.readNextTLV(byteBuf, nestingLevel + 1)) != null) {
