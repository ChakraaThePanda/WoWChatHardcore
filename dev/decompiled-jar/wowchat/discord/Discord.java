            ...
            if (channelType == null ? channelType2 != null : !((Object)((Object)channelType)).equals((Object)channelType2)) {
                ...
                if (channelType == ChannelType.PRIVATE) {
                    try {
                        ...
                        String confFile = args.length > 1 ? args[args.length - 1] : "wowchat.conf";
                        Config dmConf = ConfigFactory.parseFile(new File(confFile)).resolve();
                        if (dmConf.hasPath("dmAutoReply")) {
                            String reply = dmConf.getString("dmAutoReply");
                            if (reply != null && !reply.trim().isEmpty()) {
                                event.getChannel().sendMessage(reply.trim()).queue();
                            }
                        }
                    } catch (Throwable t) {
                        System.err.println("[Discord] Failed to send DM auto-reply: " + t.getMessage());
                    }
                }
                return;
            }
