...
extends AbstractList<E> {
    ...
    public E set(int index, E obj) {
        ...
        Object result2 = ((AVLNode)node).value;
        node.setValue(obj);
        return (E)result2;
    }
    ...
    static class AVLNode<E> {
        ...
        private AVLNode(Iterator<? extends E> iterator2, int start, int end, int absolutePositionOfParent, AVLNode<E> prev, AVLNode<E> next) {
            int mid = start + (end - start) / 2;
            if (start < mid) {
                this.left = new AVLNode<E>(iterator2, start, mid - 1, mid, prev, this);
            } else {
                this.leftIsPrevious = true;
                this.left = prev;
            }
            this.value = iterator2.next();
            this.relativePosition = mid - absolutePositionOfParent;
            if (mid < end) {
                this.right = new AVLNode<E>(iterator2, mid + 1, end, mid, this, next);
            } else {
                this.rightIsNext = true;
                this.right = next;
            }
        ...
        int indexOf(Object object, int index) {
            int result2;
            if (this.getLeftSubTree() != null && (result2 = this.left.indexOf(object, index + this.left.relativePosition)) != -1) {
                return result2;
            }
