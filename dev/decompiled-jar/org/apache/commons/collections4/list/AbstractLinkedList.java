...
implements List<E> {
    ...
    protected void doWriteObject(ObjectOutputStream outputStream2) throws IOException {
        outputStream2.writeInt(this.size());
        for (E e : this) {
            outputStream2.writeObject(e);
        }
    }

    protected void doReadObject(ObjectInputStream inputStream2) throws IOException, ClassNotFoundException {
        this.init();
        int size = inputStream2.readInt();
        for (int i = 0; i < size; ++i) {
            this.add(inputStream2.readObject());
        }
    }
    ...
    extends AbstractList<E> {
        ...
        public E remove(int index) {
            this.rangeCheck(index, this.size);
            this.checkModCount();
            E result2 = this.parent.remove(index + this.offset);
            this.expectedModCount = this.parent.modCount;
            --this.size;
            ++this.modCount;
            return result2;
