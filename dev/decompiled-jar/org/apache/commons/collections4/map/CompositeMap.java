...
implements Serializable {
    ...
    @Override
    public Set<K> keySet() {
        CompositeSet<K> keys2 = new CompositeSet<K>();
        for (int i = this.composite.length - 1; i >= 0; --i) {
            keys2.addComposited(this.composite[i].keySet());
        }
        return keys2;
    }
    ...
    public Collection<V> values() {
        CompositeCollection<V> values2 = new CompositeCollection<V>();
        for (int i = this.composite.length - 1; i >= 0; --i) {
            values2.addComposited(this.composite[i].values());
        }
        return values2;
    }
